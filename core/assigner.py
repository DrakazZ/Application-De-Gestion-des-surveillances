...
def build_canonical_structures(
    ...
    teachers = []
    teacher_index_by_id = {}
    default_submission_index = (
        int(df_profs['wish_submission_index'].max()) + 1
        if 'wish_submission_index' in df_profs.columns and not df_profs['wish_submission_index'].empty
        else len(df_profs) + 1
    )
    for idx, row in df_profs.reset_index(drop=True).iterrows():
        ...
        submission_index = row.get('wish_submission_index', default_submission_index)
        if pd_is_na(submission_index):
            submission_index = default_submission_index
    ...
    # ========== Helper structures ==========
    session_day_slot = {
        s['id']: (_date_to_weekday(s['date']), time_to_seance_code(s['time'], config.time_mapping))
        for s in sessions
    }
...
def _date_to_weekday(date_str: str) -> int:
    """Convert DD/MM date string to weekday index (0=Monday...5=Saturday)."""
    try:
        return datetime.strptime(date_str, "%d/%m").weekday()
    except ValueError:
        return -1  # invalid date → will never match any wish
    ...
    for grade, teacher_indices in teachers_by_grade.items():
        ...
        for receiver in under_assigned:
            while receiver['total_sessions'] < target_per_teacher and over_assigned:
                ...
                donor_idx = helpers['teacher_index_by_id'].get(str(donor['id']))
                receiver_idx = helpers['teacher_index_by_id'].get(str(receiver['id']))
                if donor_idx is None or receiver_idx is None:
                    break
                
                # Find a session where donor is assigned but receiver isn't
                transfer_session = find_transferable_session(donor_idx, receiver_idx, assignment, sessions, teachers, helpers)

                
                if transfer_session:
                    # Transfer the assignment
                    assignment[transfer_session].remove(donor_idx)
                    assignment[transfer_session].append(receiver_idx)
                    
                    donor['total_sessions'] -= 1
                    receiver['total_sessions'] += 1

                    invalid_transfer = False
                    if receiver['total_sessions'] > receiver.get('max_sessions', receiver['total_sessions']):
                        invalid_transfer = True
                    if assignment[transfer_session].count(receiver_idx) > 1:
                        invalid_transfer = True
                    if invalid_transfer:
                        assignment[transfer_session].remove(receiver_idx)
                        assignment[transfer_session].append(donor_idx)
                        donor['total_sessions'] += 1
                        receiver['total_sessions'] -= 1
                        break
...
def balance_grade_distribution(candidates, needed, helpers):
    ...
    for grade in sorted(grade_targets.keys(), key=lambda g: grade_targets[g], reverse=True):
        ...
        available.sort(key=lambda t: (t['total_sessions'], t.get('wish_submission_index', float('inf'))))
    ...
    if remaining > 0:
        ...
        all_remaining.sort(key=lambda t: (t['total_sessions'], t.get('wish_submission_index', float('inf'))))
...
def assign_to_session(
    ...
    allowed_present = bool(candidates['ideal'] or candidates['good'] or candidates['emergency'])
    if respect_wishes and allowed_present:
    ...
    if not allowed_present and candidates['acceptable']:
